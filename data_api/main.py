...
DATA_PATH = '/data/covertype_serving.csv'
data = []
if os.path.exists(DATA_PATH):
    with open(DATA_PATH, newline='') as csvfile:
        reader = csv.reader(csvfile)
        next(reader, None)  # Saltar el encabezado (Header)
        for row in reader:
            data.append(row)
    print(f"SUCCES: Se cargaron {len(data)} registros desde {DATA_PATH}")
else:
    print(f"ERROR: No se encontró el archivo en {DATA_PATH}. Verifique el volumen en Docker.")
