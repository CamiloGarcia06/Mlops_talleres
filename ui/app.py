...
# Detect dataset variant by inspecting session state keys
...
with st.form("predict_form"):
    if _using_pima:
        ...
        max_glu_serum = c10.selectbox(
            "Max glucosa serica (0=No medido, 1=>200, 2=300)",
            options=[0, 1, 2],
            index=int(st.session_state.get("max_glu_serum", 0)),
        )
        a1c_result = c11.selectbox(
            "HbA1c (0=No medido, 1=Normal, 2=Elevado)",
            options=[0, 1, 2],
...
if submitted:
    with st.spinner("Consultando la API..."):
        try:
            result = client.predict(features_payload)
