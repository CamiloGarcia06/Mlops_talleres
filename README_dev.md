...
| `./jupyter/notebooks` | jupyter | Notebooks de experimentacion (ejecutar desde `localhost:8888`) |
...
| Tarea | Descripcion |
|-------|-------------|
| `validate_infra` | Verifica conectividad con la API y la DB; crea la base de datos y provisiona el schema (`forest_raw`, `forest_processed`, `forest_training`) si no existen |
| `extract_raw` | Descarga un lote desde la API externa y lo guarda en `forest_raw` como JSON blob para auditoría |
| `transform_processed` | Lee `forest_raw`, mapea columnas posicionales del dataset Covertype, castea tipos numéricos y escribe en `forest_processed` |
| `clean_for_training` | Elimina nulos, duplicados y valores físicamente imposibles (Elevation ≤ 0, Cover_Type fuera de 1–7); escribe en `forest_training` |
...
├── dags/
│   ├── covertype_pipeline.py       # DAG: validate_infra → extract_raw → transform_processed → clean_for_training
...
│   └── notebooks/
│       └── train_neural_networks.ipynb  # Notebook: lee forest_training, entrena MLP (5 arquitecturas), sube a MinIO
